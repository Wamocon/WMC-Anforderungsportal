...
function getEditorSettingsDirs() {
    const home = homedir();
    const p = platform();
    const results = [];
    const editors = [
...
export function addToVSCodeSettings(settingsDir) {
    const settingsPath = join(settingsDir, 'settings.json');
    if (existsSync(settingsPath)) {
        const raw = readFileSync(settingsPath, 'utf-8');
        // Already configured?
        if (raw.includes('"anforderungsportal"'))
            return 'already';
