...
server.tool('submit_for_review', 'Submit a draft project for staff review (PO action)', { project_id: z.string().describe('Project UUID') }, async ({ project_id }) => {
    const sb = getSupabaseClient();
...
server.tool('approve_project', 'Approve a pending project (Staff/Admin action)', { project_id: z.string().describe('Project UUID') }, async ({ project_id }) => {
    const sb = getSupabaseClient();
...
server.tool('reject_project', 'Reject a pending project and send it back to draft (Staff/Admin action)', {
    project_id: z.string().describe('Project UUID'),
...
server.tool('activate_project', 'Activate an approved project for client form-filling (Staff/Admin action)', { project_id: z.string().describe('Project UUID') }, async ({ project_id }) => {
    const sb = getSupabaseClient();
...
server.tool('invite_member', 'Invite a client to a project by email', {
    project_id: z.string().describe('Project UUID'),
